...
| `M7001_TP_tokyobay.{csv,parquet}` | Tokyo Bay window (`139.55–140.30, 34.90–35.75`) | point dataset |
...
| `M7001_chart_datum_model_tokyobay.nc` | Tokyo Bay, 1′×1.5′ grid | continuous 最低水面モデル |
...
> use the Tokyo Bay product.

## Contour maps

Depth of the M7001 N-mark soundings, gridded (`scipy.griddata`, linear;
`griddata` gaps and cells > 2.5 km from any sounding are blanked) over the Tokyo
Bay window. Grey dots are the M7001 coastline (`L`, HHW) and low-tide (`M`)
waterline marks, drawn for geographic context.
